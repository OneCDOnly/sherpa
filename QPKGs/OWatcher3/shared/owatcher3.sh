...
Init()
	...
	readonly DAEMON_LAUNCH_CMD="$VENV_PYTHON_PATHFILE $DAEMON_PATHFILE --daemon --userdata $(dirname "$QPKG_INI_PATHFILE") --conf $QPKG_INI_PATHFILE --pid $DAEMON_PID_PATHFILE"
	...
	readonly GET_UI_LISTENING_ADDRESS_CMD="/opt/bin/jq -r .Server.serverhost < $QPKG_INI_PATHFILE"
...
InstallAddons()
	...
	if [[ $QPKG_NAME = OWatcher3 ]]; then
		...
		DisplayRunAndLog "KLUDGE: install 'm2r' PyPI module first" "$VENV_PIP_PATHFILE install${pip_deps} --no-input m2r" log:failure-only || SetError
	fi
