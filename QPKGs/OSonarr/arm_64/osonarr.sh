#!/usr/bin/env bash
#* don't edit this file, it was built/modified programmatically with the 'build-qpkgs.sh' script. (source: 'osonarr.source')
#* osonarr.sh
...
readonly SERVICE_SCRIPT_VERSION='240114'
Init()
{
IsQNAP || return
...
readonly QPKG_INI_PATHFILE=$QPKG_CONFIG_PATH/nzbhydra.yml
...
readonly SOURCE_GIT_URL='https://services.sonarr.tv/v1/download/main/latest?version=4&os=linux&arch='
readonly SOURCE_ARCH=arm64
...
readonly DAEMON_PATHFILE=$QPKG_REPO_PATH/Sonarr/Sonarr
readonly DAEMON_LAUNCH_CMD="export DOTNET_SYSTEM_GLOBALIZATION_INVARIANT=1; $DAEMON_PATHFILE --data=$QPKG_CONFIG_PATH --nobrowser"
...
readonly GET_UI_PORT_CMD='echo 8989'
readonly GET_UI_PORT_SECURE_CMD='echo 9898'
...
remote_archive_url="${SOURCE_GIT_URL}${SOURCE_ARCH}"
source_url_pathfile=$QPKG_REPO_PATH/source.url
local update=false
...
DisplayRunAndLog 'download source archive' '/sbin/curl${curl_insecure_arg} --show-error --location --output $local_archive_pathfile $(<$source_url_pathfile)' || { SetError; return 1 ;}
fi
if [[ ! -e $DAEMON_PATHFILE || $update = true ]]; then
DisplayRunAndLog 'extract from archive' "/bin/tar -zxvf $local_archive_pathfile --directory=$QPKG_REPO_PATH" || { SetError; return 1 ;}
fi
