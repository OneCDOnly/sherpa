...
# sickchill.sh
#
# Copyright (C) 2018-2021 OneCD [one.cd.only@gmail.com]
#
...
LoadUIPorts()
    ...
    ui_listening_address=$(/sbin/getcfg general web_host -f "$QPKG_INI_PATHFILE")
...
IsSSLEnabled()
    ...
    server_ssl_certfile=$(/sbin/getcfg general https_cert -f "$QPKG_INI_PATHFILE")
    server_ssl_keyfile=$(/sbin/getcfg general https_key -f "$QPKG_INI_PATHFILE")

    [[ -e $server_ssl_certfile && -e $server_ssl_keyfile && $(/sbin/getcfg general enable_https -d 0 -f "$QPKG_INI_PATHFILE") -eq 1 ]]
...
PullGitRepo()
    ...
    if [[ ! -d $QPKG_GIT_PATH/.git ]]; then
        ExecuteAndLog "clone $(FormatAsPackageName "$1") from remote repository" "cd /tmp; /opt/bin/git clone --branch $3 $DEPTH $GIT_HTTPS_URL $QPKG_GIT_PATH || /opt/bin/git clone --branch $3 $DEPTH -c advice.detachedHead=false $GIT_HTTP_URL $QPKG_GIT_PATH"
    else
