...
Init()
    ...
    readonly LAUNCHER="$DAEMON_PATHFILE -L info --logfile $(/usr/bin/dirname "$QPKG_INI_PATHFILE")/$QPKG_NAME.log --config $(/usr/bin/dirname "$QPKG_INI_PATHFILE")/ --pidfile $DAEMON_PID_PATHFILE"
    readonly PORT_CHECK_TIMEOUT=60
    readonly DAEMON_STOP_TIMEOUT=60
    readonly DAEMON_PORT_CMD="/opt/bin/jq -r .daemon_port < $QPKG_INI_PATHFILE | /usr/bin/tail -n1"
    readonly UI_PORT_CMD=''
    readonly UI_PORT_SECURE_CMD=''
    readonly UI_PORT_SECURE_ENABLED_TEST_CMD='false'
    readonly UI_LISTENING_ADDRESS_CMD="/opt/bin/jq -r .listen_interface < $QPKG_INI_PATHFILE | /usr/bin/tail -n1"
    daemon_port=0
    ui_port=0
    ui_port_secure=0
