...
Init()
	...
	readonly QPKG_INI_PATHFILE=$QPKG_CONFIG_PATH/core.conf
	...
	readonly DAEMON_PATHFILE=/opt/bin/deluged
	readonly DAEMON_LAUNCH_CMD=". $VENV_PATH/bin/activate && $VENV_INTERPRETER $DAEMON_PATHFILE -L info --logfile $(/usr/bin/dirname "$QPKG_INI_PATHFILE")/$QPKG_NAME.log --config $(/usr/bin/dirname "$QPKG_INI_PATHFILE")/ --pidfile $DAEMON_PID_PATHFILE"
	...
	readonly GET_DAEMON_PORT_CMD="/opt/bin/jq -r .daemon_port < $QPKG_INI_PATHFILE | /usr/bin/tail -n1"
	readonly GET_UI_PORT_CMD=''
	readonly GET_UI_PORT_SECURE_CMD=''
	readonly GET_UI_PORT_SECURE_ENABLED_TEST_CMD='false'
	readonly GET_UI_LISTENING_ADDRESS_CMD="/opt/bin/jq -r .listen_interface < $QPKG_INI_PATHFILE | /usr/bin/tail -n1"
