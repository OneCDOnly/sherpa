...
Init()
	...
	readonly DAEMON_PATHFILE=$QPKG_REPO_PATH/Mylar.py
	readonly DAEMON_LAUNCH_CMD="$VENV_PYTHON_PATHFILE $DAEMON_PATHFILE --daemon --nolaunch --datadir $(/usr/bin/dirname "$QPKG_INI_PATHFILE") --config $QPKG_INI_PATHFILE --pidfile $DAEMON_PID_PATHFILE"
	...
	readonly GET_UI_PORT_CMD="/sbin/getcfg interface http_port -d 0 -f $QPKG_INI_PATHFILE"
	readonly GET_UI_PORT_SECURE_CMD="/sbin/getcfg interface http_port -d 0 -f $QPKG_INI_PATHFILE"
	readonly GET_UI_PORT_SECURE_ENABLED_TEST_CMD='[[ $(/sbin/getcfg interface enable_https -d 0 -f '$QPKG_INI_PATHFILE') = 1 ]]'
	readonly GET_UI_LISTENING_ADDRESS_CMD="/sbin/getcfg interface http_host -d undefined -f $QPKG_INI_PATHFILE"
