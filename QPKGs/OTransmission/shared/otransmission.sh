...
LoadUIPorts()
    ...
    case $1 in
        app)
            ...
            ui_port=$(/opt/bin/jq -r '."rpc-port"' < "$QPKG_INI_PATHFILE")
    ...
    ui_listening_address=$(/opt/bin/jq -r '."rpc-bind-address"' < "$QPKG_INI_PATHFILE")
...
LoadAppVersion()
    ...
    [[ -n $DAEMON_PATHFILE && -e $DAEMON_PATHFILE ]] && app_version=$($DAEMON_PATHFILE --version 2>&1 | /bin/sed "s|$(/usr/bin/basename $DAEMON_PATHFILE)||")
