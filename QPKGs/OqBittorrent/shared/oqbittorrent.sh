...
Init()
	...
	readonly QPKG_INI_PATHFILE=$QPKG_CONFIG_PATH/qBittorrent/config/qBittorrent.conf
	...
	readonly ORIG_DAEMON_SERVICE_SCRIPT=/opt/etc/init.d/S89qbittorrent

	# specific to daemonised applications only
	readonly DAEMON_PATHFILE=/opt/bin/qbittorrent-nox
	readonly DAEMON_LAUNCH_CMD="$DAEMON_PATHFILE --profile=$QPKG_PATH/config --daemon"
	...
	readonly GET_UI_PORT_CMD="/sbin/getcfg Preferences 'WebUI\Port' -d 0 -f $QPKG_INI_PATHFILE"
	readonly GET_UI_PORT_SECURE_CMD="/sbin/getcfg Preferences 'WebUI\Port' -d 0 -f $QPKG_INI_PATHFILE"
