...
Init()
	...
	readonly SOURCE_ARCH=linux-arm32v5
...
StartQPKG()
	...
	if [[ ! -e $DAEMON_PATHFILE || $update = true ]]; then	# download latest release archive from GitHub & extract all files
		...
		local remote_archive_url=$(/sbin/curl${curl_insecure_arg} --silent "$SOURCE_GIT_URL" | /bin/grep browser_download_url | cut -d\" -f4 | /bin/grep "$SOURCE_ARCH")
		local local_archive_pathfile=$QPKG_PATH/olivetin.tar.gz
		...
		DisplayRunAndLog 'download source archive' "/sbin/curl${curl_insecure_arg} --silent --show-error --location --output $local_archive_pathfile $remote_archive_url" || { SetError; return 1 ;}
 		DisplayRunAndLog 'extract from archive' "/bin/tar -zxvf $local_archive_pathfile --strip-components=1 -C $QPKG_REPO_PATH" || { SetError; return 1 ;}
	fi
