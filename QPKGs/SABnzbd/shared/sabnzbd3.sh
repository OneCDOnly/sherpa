...
Init()
    ...
    readonly LAUNCHER="cd $QPKG_REPO_PATH; $INTERPRETER $DAEMON_PATHFILE --daemon --browser 0 --config-file $QPKG_INI_PATHFILE --pidfile $DAEMON_PID_PATHFILE"
...
LoadUIPorts()
    ...
    ui_listening_address=$(/sbin/getcfg misc host -f "$QPKG_INI_PATHFILE")
...
IsSSLEnabled()
    ...
    [[ $(/sbin/getcfg misc enable_https -d 0 -f "$QPKG_INI_PATHFILE") -eq 1 ]]
