...
EnsureConfigFileExists()
    ...
    if [[ $($GETCFG_CMD Deluge-server Enable -d FALSE -f $APP_CENTER_CONFIG_PATHFILE) = TRUE ]]; then
        web_auth_pathfile=$($DIRNAME_CMD "$QPKG_INI_PATHFILE")/auth
        server_auth_pathfile=$($GETCFG_CMD Deluge-server Install_Path -f "$APP_CENTER_CONFIG_PATHFILE")/config/auth
