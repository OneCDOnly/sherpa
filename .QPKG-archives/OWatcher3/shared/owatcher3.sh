...
Init()
    ...
    # for Python-based remote apps
        ...
        readonly TARGET_SCRIPT=watcher.py
    ...
    readonly APP_VERSION_PATHFILE=$QPKG_REPO_PATH/core/version.py
    ...
    if [[ -n $PYTHON && -n $TARGET_SCRIPT ]]; then
        readonly LAUNCHER="cd $QPKG_REPO_PATH; $PYTHON $TARGET_SCRIPT_PATHFILE --daemon --userdata $($DIRNAME_CMD "$QPKG_INI_PATHFILE") --conf $QPKG_INI_PATHFILE --pid $DAEMON_PID_PATHFILE"
    else
...
LoadUIPorts()
    ...
    case $1 in
        app)
            ...
            ui_port=$($JQ_CMD -r .Server.serverport < "$QPKG_INI_PATHFILE")
            ui_port_secure=$ui_port
            ;;
        qts)
    ...
    ui_listening_address=$($JQ_CMD -r .Server.serverhost < "$QPKG_INI_PATHFILE")
...
IsSSLEnabled()
    ...
    server_ssl_certfile=$($JQ_CMD -r .Server.ssl_cert < "$QPKG_INI_PATHFILE")
    server_ssl_keyfile=$($JQ_CMD -r .Server.ssl_key < "$QPKG_INI_PATHFILE")
