...
Display "$(ShowTitle) $(ShowVersion) $(ShowDescription)"
...
IsSysFileExist $QTS_SLEEP_CMD || return
fi
...
readonly THIS_SCRIPT_VER="230825-$SOURCE_BRANCH"
...
readonly KERNEL_PAGESIZE=$(GetKernelPageSize)
...
rm -rf "$IPK_DL_PATH" "$IPK_CACHE_PATH" "$PIP_CACHE_PATH"
...
if [[ $KERNEL_PAGESIZE = '4kB' ]]; then
DebugFirmware ok 'kernel page size' "$KERNEL_PAGESIZE"
else
DebugFirmware warning 'kernel page size' "$KERNEL_PAGESIZE"
fi
...
DebugQpkg detect "$(ShowAsPackageName Entware)" "$ENTWARE_VER"
...
if QPKG.IsEnabled Entware; then
ipks_upgrade=true
ipks_install=true
pips_install=true
...
if QPKGs-ACupgrade-to.Exist sherpa; then
QPKGs-ACreactivate-to:Add sherpa
fi
...
if QPKGs-AC${action}-to.IsAny && QPKGs-ISenabled.Exist Entware; then
ipks_upgrade=true
ipks_install=true
pips_install=true
break
...
[[ $opts_verbose = true ]] && QPKGs.States:List rebuild
Func:Exit
}
Action:Proc()
{
...
;;
IPK|PIP)
if [[ $ipks_upgrade = true || $ipks_install = true || $pips_install = true ]]; then
...
;;
activate|deactivate|enable|disable|reactivate|stop|uninstall)
case $group in
all|dependent|standalone)
found=true
for prospect in $(QPKGs-SC${group}:Array); do
...
ShowAsIncrementalProgress 'resolving IPK dependencies' "$iterations" iteration "${#dep_acc[@]}" package
((iterations++))
...
ShowAsFail "upgrade $total_count IPK$(Pluralise "$total_count") failed $(ShowAsExitcode "$result_code")"
...
ShowAsFail "install $desc failed $(ShowAsExitcode "$result_code")"
...
((fail_count++))
ShowAsFail "install $desc failed $(ShowAsResult "$result_code")"
...
DisplayAsProjSynIndentExam "list $(ShowTitle) object version numbers" 'list versions'
...
Help.Action.Reassign:Show()
{
DisplayAsProjSynIndentExam "reassign packages to $(ShowTitle). Use this action to detach QPKGs previously installed via an online repository from further management by that repository" "reassign $(ShowPackages)"
}
Help.ActionsAll:Show()
{
DisableDebugToArchiveAndFile
...
DisplayAsHelpTitle "$(ShowAction) $(ShowGroup) usage examples:"
...
Help.Basic:Show()
{
DisplayAsHelpTitle "usage: sherpa $(ShowAction) $(ShowPackages) $(ShowGroup) $(ShowOptions)"
return 0
}
Help.Basic.Examples:Show()
{
DisplayAsProjSynIndentExam "to list available $(ShowAction)s" 'list actions'
DisplayAsProjSynIndentExam "to list available $(ShowPackages)" 'list packages'
DisplayAsProjSynIndentExam '' p
DisplayAsProjSynIndentExam "to list available $(ShowGroup)s" 'list groups'
DisplayAsProjSynIndentExam "or, for more $(ShowOptions)" 'list options'
...
DisableDebugToArchiveAndFile
Help.Basic:Show
DisplayAsHelpTitle "$(ShowGroup) usage examples:"
...
DisplayAsProjSynExam 'multiple groups are supported like this' "$(ShowAction) $(ShowGroup) $(ShowGroup)"
return 0
}
Help.Issue:Show()
{
...
DisplayAsProjSynIndentExam "view only the most recent $(ShowTitle) session log" last
DisplayAsProjSynIndentExam "view the entire $(ShowTitle) session log" log
DisplayAsProjSynIndentExam "upload the most-recent $(FormatAsThous "$LOG_TAIL_LINES") lines in your $(ShowTitle) log to the $(ShowAsURL 'https://termbin.com') public pastebin. A URL will be generated afterward" 'paste log'
DisplayAsHelpTitleHighlighted "if you need help, please include a copy of your $(ShowTitle) $(ColourTextBrightOrange 'log for analysis!')"
return 0
}
...
DisplayAsProjSynIndentExam '' c
DisplayAsProjSynIndentExam 'clear local repository files from these packages' "clean $(ShowPackages)"
DisplayAsProjSynIndentExam "remove all cached $(ShowTitle) items and logs" reset
...
DisplayAsProjSynIndentExam "view the entire $(ShowTitle) session log" log
DisplayAsProjSynIndentExam "upload the most-recent session in your $(ShowTitle) log to the $(ShowAsURL 'https://termbin.com') public pastebin. A URL will be generated afterward" 'paste last'
DisplayAsProjSynIndentExam "upload the most-recent $(FormatAsThous "$LOG_TAIL_LINES") lines in your $(ShowTitle) log to the $(ShowAsURL 'https://termbin.com') public pastebin. A URL will be generated afterward" 'paste log'
DisplayAsHelpTitleHighlighted "if you need help, please include a copy of your $(ShowTitle) $(ColourTextBrightOrange "log for analysis!")"
return 0
}
...
DisplayAsProjSynIndentExam "install all available $(ShowTitle) packages" 'install all'
...
ExtractPrevSessFromTail
if [[ -e $SESS_LAST_PATHFILE ]]; then
if Quiz "Press 'Y' to post the most-recent session in your $(ShowTitle) log to a public pastebin, or any other key to abort"; then
ShowAsProc "uploading $(ShowTitle) log"
link=$($CAT_CMD --number "$SESS_LAST_PATHFILE" | (exec 3<>/dev/tcp/termbin.com/9999; $CAT_CMD >&3; $CAT_CMD <&3; exec 3<&-))
if [[ $? -eq 0 ]]; then
ShowAsDone "your $(ShowTitle) log is now online at $(ShowAsURL "$link") and will be deleted in 1 month."
else
...
ExtractTailFromLog
if [[ -e $SESS_TAIL_PATHFILE ]]; then
if Quiz "Press 'Y' to post the most-recent $(FormatAsThous "$LOG_TAIL_LINES") lines in your $(ShowTitle) log to a public pastebin, or any other key to abort"; then
ShowAsProc "uploading $(ShowTitle) log"
...
ShowAsError "the '$package' QPKG is enabled. $(ShowTitle) is incompatible with this package. Please consider 'stop'ing this QPKG in your App Center."
return 1
fi
done
fi
...
if QPKG.IsEnabled "$package"; then
ShowAsWarn "the '$package' QPKG is enabled. This may cause problems with $(ShowTitle) applications. Please consider 'stop'ing this QPKG in your App Center."
fi
done
fi
return 0
...
{
Help.Basic:Show
DisplayAsHelpTitle "$(ShowTitle) can recognise various abbreviations and aliases as $(ShowPackages)."
DisplayAsAbsReportTitleLine
...
DisplayWritingReport
{
DisplayAsHelpTitle "the location for $(ShowTitle) backups is: $QPKG_BU_PATH"
...
DisplayWritingReport
{
DisplayAsHelpTitle "$(ShowTitle) will automatically install and start/stop dependencies as-required."
...
DisplayAsProjSynExam "abbreviations and aliases may also be used to specify $(ShowPackages). To list these" 'list abs'
DisplayAsProjSynIndentExam '' a
} > "$REPORT_PATHFILE"
WriteEraseThisLine
[[ -e $REPORT_PATHFILE ]] && $CAT_CMD "$REPORT_PATHFILE"
...
GetFirmwareVer()
{
/sbin/getcfg System Version -d none -f /etc/config/uLinux.conf
}
GetFirmwareBuild()
{
...
QPKG.GetInstallDate()
{
/sbin/getcfg "${1:?${FUNCNAME[0]}'()': undefined}" date -f /etc/config/qpkg.conf
return 0
}
QPKG.IsBackupExist()
{
...
ShowAsCommand()
{
echo -n "${CHARS_RESULTS}command: '${1:?${FUNCNAME[0]}'()': undefined}'"
}
ShowAsResult()
{
[[ -n ${1:-} ]] || return
...
DisplayAsIndentPrefixActionResultDurationReason '' "$3" "$2" '' "no reason was provided by $(ShowTitle)"
fi
;;
failed)
if [[ -n "${6:-}" ]]; then
...
StripANSI()
{
if [[ -e $GNU_SED_CMD && -L /opt/etc/passwd ]]; then
