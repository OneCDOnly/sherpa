#!/usr/bin/env bash
#* don't edit this file, it was built/modified programmatically with the `build-manager.sh` script. (source: sherpa-manager.source)
#* sherpa-manager.sh
...
readonly PACKAGE_ENABLED_COL_WIDTH=17
...
readonly OBJECTS_ARCHIVE_URL='https://raw.githubusercontent.com/OneCDOnly/sherpa/unstable'/objects.tar.gz
readonly PACKAGES_ARCHIVE_URL='https://raw.githubusercontent.com/OneCDOnly/sherpa/unstable'/packages.tar.gz
...
readonly THIS_SCRIPT_VER='230915-unstable'
...
QPKGs-ACinstall-to.Exist Entware && QPKGs-ACreactivate-to:Add Entware
...
total_count=$(IPKs-ACdownload-to:Count)
if [[ $total_count -gt 0 ]]; then
local desc="$total_count complementary IPK module$(Pluralise "$total_count")"
...
local desc='complementary PyPI modules'
...
if [[ $maxcols -ge 3 ]]; then
package_enabled_msg="${CHARS_SPECIAL}QPKG enabled?"
printf "%-$((PACKAGE_ENABLED_COL_WIDTH+2+$(LenANSIDiff "$package_enabled_msg")))s" "$package_enabled_msg"
fi
if [[ $maxcols -ge 4 ]]; then
...
package_enabled_msg+="$(ColourTextBrightGreen true)"
else
package_enabled_msg+="$(ColourTextBrightRed '!false')"
fi
if QPKG.IsArchOK "$package_name"; then
...
package_enabled_msg+="$(ColourTextDarkGrey 'N/A')"
if QPKG.IsArchOK "$package_name"; then
...
DisplayFileInWindow "$REPORT_PATHFILE" onescreen
else
...
DisplayFileInWindow "$SESS_LAST_PATHFILE" linenumbers onescreen
else
...
DisplayFileInWindow "$SESS_TAIL_PATHFILE" linenumbers onescreen
else
...
{
/sbin/getcfg "${1:-$PACKAGE_NAME}" Shell -d undefined -f /etc/config/qpkg.conf
}
QPKG.Avail.GetVer()
{
local -i index=0
...
QPKG.IsRepoOk()
{
local temp="$(QPKG.GetStoreID "${1:-$PACKAGE_NAME}")"
[[ -z $temp || $temp = sherpa ]]
}
QPKG.GetInstallDate()
{
...
QPKG.IsArchOK()
{
local temp="$(QPKG.GetURL "${1:-$PACKAGE_NAME}")"
[[ -n $temp && $temp != none ]]
}
QPKG.GetMinRAM()
{
...
local -i index=0
for index in "${!QPKG_NAME[@]}"; do
if [[ ${QPKG_NAME[$index]} = "${1:-$PACKAGE_NAME}" ]]; then
